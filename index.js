 ...
 */
const traverse = function(ast, visitorEnterOrActions) {
	var visitorActions = {};
...
traverse.STOP_TRAVERSAL = 'stop'
traverse.SKIP_NODES_BELOW = 'skip'
...
function isASTNode (node, name) {
	return (
		...
		node.hasOwnProperty ('nodeType') &&	//a 'type' key must exist in the node
		typeof (node.nodeType) === 'string' &&	//node.type's value must be a string
...
Controller.prototype.skipNodesBelow = function skip () {
	this.notify (traverse.SKIP_NODES_BELOW);
};
...
Controller.prototype.stopTraversal = function stop () {
	this.notify (traverse.STOP_TRAVERSAL);
};
 ...
 * @private
 */
Controller.prototype.traverse = function(root, parent, visitorActions) {
	if (!isASTNode (root) ||
		this.__flag === traverse.STOP_TRAVERSAL) {
	...
	if (ret === traverse.STOP_TRAVERSAL) {
		
		this.notify (ret);
		return;

	} else if (!(ret === traverse.SKIP_NODES_BELOW ||
		this.__flag === traverse.SKIP_NODES_BELOW)) {
	...
	if (this.__flag !== traverse.STOP_TRAVERSAL) {
